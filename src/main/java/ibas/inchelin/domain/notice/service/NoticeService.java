...
public class NoticeService {
    ...
    // 공지 목록 조회
    public List<NoticeListResponse.NoticeListDto> findNotices() {
        List<Notice> notices = noticeRepository.findAll(); //DB에서 모든 공지를 조회
        return notices.stream()
                .map(NoticeListResponse.NoticeListDto::new)
                .collect(Collectors.toList());
    }

    // 공지 내용 조회
    public NoticeContentResponse.NoticeDetailDto findNoticeContent(Long noticeId) {
        // DB에서 noticeId에 해당하는 공지를 조회. 없으면 예외 발생
        Notice notice = noticeRepository.findById(noticeId)
                .orElseThrow(() -> new IllegalArgumentException("해당 공지를 찾을 수 없습니다."));

        return new NoticeContentResponse.NoticeDetailDto(notice);
    }
}
